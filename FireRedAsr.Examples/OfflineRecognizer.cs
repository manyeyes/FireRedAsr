...
namespace FireRedAsr.Examples
{
    ...
    {
        private static FireRedAsr.OfflineRecognizer? _offlineRecognizer;
        ...
        public static void OfflineRecognizer(string streamDecodeMethod = "one", string modelName = "fireredasr2-aed-large-zh-en-int8-onnx-offline-20260212", string modelAccuracy = "int8", int threadsNum = 2, string[]? mediaFilePaths = null, string? modelBasePath = null)
        {
            ...
            if (streamDecodeMethod == "one")
            {
                ...
                try
                {
                    ...
                    foreach (var sample in samples)
                    {
                        ...
                        r.Append($"\"word\":[{string.Join(",", result.Tokens.Select(x => $"\"{x}\"").ToArray())}],");
            ...
            if (streamDecodeMethod == "multi")
            {
                ...
                try
                {
                    ...
                    List<FireRedAsr.Model.OfflineRecognizerResultEntity> results = offlineRecognizer.GetResults(streams);
                    foreach (FireRedAsr.Model.OfflineRecognizerResultEntity result in results)
                    {
                        Console.WriteLine($"{paths[n]}");
                        StringBuilder r = new StringBuilder();
                        r.Append("{");
